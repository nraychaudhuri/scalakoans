package org.functionalkoans.forscala

import support.KoanSuite

class AboutTraits extends KoanSuite {
  koan("A class uses the extends keyword to mixin a trait if it is the only relationship the class inherits") {
    case class Event(name: String)
    trait EventListener {
      def listen(event: Event): String
    }
    class MyListener extends EventListener {
      def listen(event: Event): String = {
        event match {
          case Event("Moose Stampede") => "An unfortunate moose stampede occurred"
          case _ => "Nothing of importance occurred"
        }
      }
    }
  
    val evt = Event("Moose Stampede")
    val myListener = new MyListener
    myListener.listen(evt) should be (__)
  }
  
  koan("A class can only \'extend\' from one class or trait, any subsequent extension should use the keyword \'with\'") {
  
    case class Event(name: String)
  
    trait EventListener {
      def listen(event: Event): String
    }
  
    class OurListener
  
    class MyListener extends OurListener with EventListener {
      def listen(event: Event) : String = {
        event match {
          case Event("Woodchuck Stampede") => "An unfortunate woodchuck stampede occurred"
          case _ => "Nothing of importance occurred"
        }
      }
    }
  
    val evt = Event("Woodchuck Stampede")
    val myListener = new MyListener
    myListener.listen(evt) should be (__)
  }
  
  koan("Traits are polymorphic. Any type can be referred to by another type if related by extension") {
    case class Event(name: String)
    trait EventListener {
      def listen(event: Event): String
    }
    class MyListener extends EventListener {
      def listen(event: Event) : String = {
        event match {
          case Event("Moose Stampede") => "An unfortunate moose stampede occurred"
          case _ => "Nothing of importance occurred"
        }
      }
    }
  
    val evt = Event("Moose Stampede")
    val myListener = new MyListener
  
    myListener.isInstanceOf[MyListener] should be(__)
    myListener.isInstanceOf[EventListener] should be(__)
    myListener.isInstanceOf[Any] should be(__)
    myListener.isInstanceOf[AnyRef] should be(__)
  }
  
  koan("Traits can have concrete implementations that can be mixed into concrete classes with it's own state") {
    trait Logging {
      val logCache = scala.collection.mutable.ListBuffer[String]()
  
      def log(value: String) = {
        logCache += value
        println(value)
      }
    }
  
    class Welder extends Logging {
      def weld() {
        log("welding pipe")
      }
    }
  
    class Baker extends Logging {
      def bake() {
        log("baking cake")
      }
    }
  
    val welder = new Welder
    welder.weld
  
  
    val baker = new Baker
    baker.bake
  
    welder.logCache.size should be(__)
    baker.logCache.size should be(__)
  }
  
  koan("Traits are instantiated before a classes instantiation") {
    val sb = scala.collection.mutable.ListBuffer[String]()
  
    trait T1 {
      sb += ("In T1: x=%s".format(x))
      val x = 1
      sb += ("In T1: x=%s".format(x))
    }
  
    class C1 extends T1 {
      sb += ("In C1: y=%s".format(y))
      val y = 2
      sb += ("In C1: y=%s".format(y))
    }
  
    sb += ("Creating C1")
    new C1
    sb += ("Created C1")
  
    //Tip: use REPL to find the final string
    sb.mkString(";") should be(__)
  }
  
  
  koan("Traits are instantiated before a classes instantiation from left to right") {
    val sb = scala.collection.mutable.ListBuffer[String]()
  
    trait T1 {
      sb += ("In T1: x=%s".format(x))
      val x = 1
      sb += ("In T1: x=%s".format(x))
    }
  
    trait T2 {
      sb += ("In T2: z=%s".format(z))
      val z = 1
      sb += ("In T2: z=%s".format(z))
    }
  
    class C1 extends T1 with T2 {
      sb += ("In C1: y=%s".format(y))
      val y = 2
      sb += ("In C1: y=%s".format(y))
    }
  
    sb += ("Creating C1")
    new C1
    sb += ("Created C1")
  
    sb.mkString(";") should be(__)
  }
  
  koan("Instantiations are tracked and will not allow a duplicate instantiation. " +
    "Note T1 extends T2, and C1 also extends T2, but T2 is only instantiated once.") {
  
    val sb = scala.collection.mutable.ListBuffer[String]()
  
    trait T1 extends T2 {
      sb += ("In T1: x=%s".format(x))
      val x = 1
      sb += ("In T1: x=%s".format(x))
    }
  
    trait T2 {
      sb += ("In T2: z=%s".format(z))
      val z = 1
      sb += ("In T2: z=%s".format(z))
    }
  
    class C1 extends T1 with T2 {
      sb += ("In C1: y=%s".format(y))
      val y = 2
      sb += ("In C1: y=%s".format(y))
    }
  
    sb += ("Creating C1")
    new C1
    sb += ("Created C1")
  
    sb.mkString(";") should be(__)
  }
  
  
  koan("The diamond of death (http://en.wikipedia.org/wiki/Diamond_problem) is avoided since " +
    "instantiations are tracked and will not allow multiple instantiations") {
  
    val sb = scala.collection.mutable.ListBuffer[String]()
  
    trait T1 {
      sb += ("In T1: x=%s".format(x))
      val x = 1
      sb += ("In T1: x=%s".format(x))
    }
  
    trait T2 extends T1 {
      sb += ("In T2: z=%s".format(z))
      val z = 2
      sb += ("In T2: z=%s".format(z))
    }
  
    trait T3 extends T1 {
      sb += ("In T3: w=%s".format(w))
      val w = 3
      sb += ("In T3: w=%s".format(w))
    }
  
    class C1 extends T2 with T3 {
      sb += ("In C1: y=%s".format(y))
      val y = 4
      sb += ("In C1: y=%s".format(y))
    }
  
    sb += ("Creating C1")
    new C1
    sb += ("Created C1")
  
    sb.mkString(";") should be(__)
  }
  
  koan("Who is going to win?") {
  
    trait Winner { def win: String }
    trait A extends Winner { override def win = "A is the winner" }
    trait B extends Winner { override def win = "B is the winner" }
    trait C extends Winner { override def win = "C is the winner" }
  
    val w = new A with B with C { }
  
    w.win should be(__)
  }
  
  koan("Traits are stackable") {
    trait WhoAmI { def name: String }
    trait A extends WhoAmI { override abstract def name = "A -> " + super.name }
    trait B extends WhoAmI { override abstract def name = "B -> " + super.name }
    trait C extends WhoAmI { override abstract def name = "C -> " + super.name }
    
    class D extends WhoAmI {
      override def name = "D"
    }
    
    val confused = new D with A with B with C     
    confused.name should be(__)

    val another = new D with C with B with A    
    another.name should be(__)
  }

}
